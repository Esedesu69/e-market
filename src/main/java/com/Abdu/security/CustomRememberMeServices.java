...
public class CustomRememberMeServices extends PersistentTokenBasedRememberMeServices {
    ...
    private PersistentTokenRepository tokenRepository = new InMemoryTokenRepositoryImpl(); // !!!! could be changed to be stored in real database !!
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication successfulAuthentication) {
        String username = ((AppUser) successfulAuthentication.getPrincipal()).getUsername();
        System.out.println("Creating new persistent login for user " + username);
    ...
    protected Authentication createSuccessfulAuthentication(HttpServletRequest request, UserDetails user) {
        AccountEntity auser = userRepository.findByEmail(user.getUsername());
